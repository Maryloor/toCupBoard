  ...
  if ( ! is_active_sidebar( 'sidebar-1' )) { ?>
	<aside id="secondary" class="widget-area">
		<section id="Search" class="widget widget_block widget_archive " >
		    <h2 class="widget-title"><?php esc_html_e('Search', 'coffee-bistro'); ?></h2>
		    <?php get_search_form(); ?>
		</section>
		<section id="archives" class="widget widget_block widget_archive " >
		    <h2 class="widget-title"><?php esc_html_e('Archives', 'coffee-bistro'); ?></h2>
		    <ul>
		        <?php
		...
		<section id="categories" class="widget widget_categories" role="complementary">
		    <h2 class="widget-title"><?php esc_html_e('Categories', 'coffee-bistro'); ?></h2>
		    <ul>
		        <?php
		...
		<section id="tags" class="widget widget_tag_cloud" role="complementary">
		    <h2 class="widget-title"><?php esc_html_e('Tags', 'coffee-bistro'); ?></h2>
		    <?php
				$coffee_bistro_tags = get_tags();
				if ($coffee_bistro_tags) {
				    echo '<div class="tag-cloud">';
				    foreach ($coffee_bistro_tags as $tag) {
		...
		<section id="recent-posts" class="widget" role="complementary">
		    <h2 class="widget-title"><?php esc_html_e('Recent Posts', 'coffee-bistro'); ?></h2>
		    <ul class="recent-posts-list">
		        <?php
		        $coffee_bistro_recent_posts = get_posts(array(
		            'numberposts' => 5, // Adjust the number of posts to display
		            'post_status' => 'publish',
		        ));

		        foreach ($coffee_bistro_recent_posts as $post) :
		            setup_postdata($post);
		            ?>
		            <li>
