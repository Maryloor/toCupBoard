	...
	class WPTT_WebFont_Loader {
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$coffee_bistro_style = explode( '}', $font_face )[0];

				// Sanity check.
				if ( false === strpos( $coffee_bistro_style, 'font-family' ) ) {
					continue;
				}

				// Get an array of our font-families.
				preg_match_all( '/font-family.*?\;/', $coffee_bistro_style, $matched_font_families );

				// Get an array of our font-files.
				preg_match_all( '/url\(.*?\)/i', $coffee_bistro_style, $matched_font_files );
